...
export default class DecodeView extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      reviewQuestions: this.filterQuestions(this.props.navigation.state.params.group._typeObject),
      countDownTime: 0,
  ...
  render() {
    const {
      viewMode, initialStoryTitle, initialStoryDescription, reviewQuestions, reviewIndex, storyHeightAnim, videoFlexAnim,
                ...
                <Animated.View style={[styles.questionLayerContent, { opacity: questionWrapperOpacityAnim, flex: 1 }]}>
                  {viewMode === ViewModes[0]
                    ...
                    : viewMode === ViewModes[2]
                      ? (
                        <View>
                          {reviewQuestions[reviewIndex] && reviewQuestions[reviewIndex].content3.map((answer => {
                            return (
                              <TouchableOpacity style={styles.answerListItem} key={answer.id} onPress={() => this.changeAnswer(answer.id)}>
                                <View style={{ marginRight: 20 }}>
                                  <RadioIcon checked={currentAnswerId === answer.id} />
                                </View>
                                <View>
                          ...
                          <View style={styles.buttonGroup}>
                            ...
                            <TouchableOpacity onPress={() => this.changeViewMode(ViewModes[0])} style={styles.button}>
