          ...
          facebookLink = async () => {
            ...
            const getFacebookInfoCallback = (error, result) => {
              ...
              if (error) {
                ...
                console.log("mcmcmmc",error)
        
                // LoginView.flashMessage('Can not fetch your Facebook profile');
              } else {
                userActions.facebookLinkRequest(this.props.user._id,result);
                console.log("resilllll",result)
                // this.props.navigation.dispatch(facebookLoginWitthEmail(this.props.user._id)) 
                // bugsnag.notify(JSON.stringify(result));
              }
            };
