...
import * as groupsutil from "../utils/groupsutil";
...
class StoryView extends Component {
    ...
    getUserAchievements = async () => {
        ...
        let response = await instance(
            USER_ACHIEVEMENT_LIST_PATH_API.replace(user._id)
        );
    ...
    _loadStories = (user, stories) => {
        ...
        const filteredStories = filterStories(stories.lstStories, user);
        this.setState({
    ...
    refreshUserTask = () => {
        // USER_TASK_GROUP_LIST_API
            // console.log('insideworldworldworldworldworld',this.props.world.groups.recommendGroups)
      let getGroupByUserData  = groupsutil.getGroupByUser(this.props.world.groups.recommendGroups,this.props.userProfile._id)
            //    console.log('response.data.latestUserTaskGroups22',getGroupByUserData)
            if (getGroupByUserData) {
                ...
                if (getGroupByUserData.length >= 3) {
                    //console.log('has groups');
                    this.setState({
                        isStoryLocked: true
                    })
                } else {
                    //console.log('has no groups');
                    this.setState({
                        isStoryLocked: false
                    })
                }
        ...
        // instance.get(`/userTaskGroupWithMessage?user_email=${this.props.user.profile.email}`)
        //     .then((response) => {
        //         console.log('response.data.latestUserTaskGroups',response)

        //         if (response) {

        //             if (response.data) {
        //                 if (response.data.latestUserTaskGroups) {
        //                     this.setState({userTaskGroups: response.data.latestUserTaskGroups})
        //                     this.props.userActions.updateUserTaskGroup(response.data.latestUserTaskGroups);
        //                     if (response.data.latestUserTaskGroups.length >= 3) {
        //                         //console.log('has groups');
        //                         this.setState({
        //                             isStoryLocked: true
        //                         })
        ...
        //     })
            ...
            // .catch((error) => console.log("eeeeeeeeeee",error))
    }
    ...
    componentDidMount() {
        if (this.props.user) {
            ...
            //this.getUserAchievements(); //no need to fetch stories from api, just get it from redux.
            this._loadStories(user, this.props.world.stories);
        ...
        if (!this.state.processing) {
            ...
            fetch(TEAM_UPDATE_API.replace('{}', teamId), {
                ...
                .then(response => {
                    ...
                    console.log('userTaskGroup', response);
                    console.log(this.props.user);
                    console.log(response._team);
                        ...
                        />
                        {
                            ...
                            <View style={{flex: 1,justifyContent : 'flex-end',alignItems :'center'}}>
                             ...
                             <View style={{flex: 1,justifyContent : 'center',alignItems :'center'}}>
                                <Text style={{color: '#ffffff', fontFamily: fontFamilyName, fontSize: 16}}>You have hit the cap of 3 groups</Text>
                               </View>
                               <View style={{flex: 1,justifyContent : 'flex-start',alignItems :'center'}}>
                                <TouchableOpacity style={{ paddingHorizontal: 16, paddingVertical: 10, backgroundColor: '#1FBEB8',height : 42,borderRadius:21,justifyContent : 'center',alignItems: 'center' }} onPress={()=>{
                                    this.props.navigation.navigate('UserTaskGroup');
                                }}>
                                <Text style={{textAlign: 'center', fontSize: 18, fontFamily: fontFamilyName, color: 'white'}}>Go to My Groups</Text>
                                </TouchableOpacity>
                              </View>
                            </View>
                            </View>
                        ...
                        <View style={styles.likeModalInnerView}>
                            ...
                            {this.state.tasksFetching ? (
                                ...
                                <View>
                                    ...
                                    <TouchableOpacity onPress={() => this.createNewTeam()}>
                                        <Text
                                            style={{
                                                ...styles.likeModalAction,
                                                ...{
