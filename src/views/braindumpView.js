...
export default class BrainDumpView extends Component {
  constructor(props) {
    super(props);
    const {
  ...
  uploadImage(photo) {
    ...
    fetch(path, {
      ...
      .then(response => {
        console.log('upload succesddddddddddddddddd', response);
        this.saveBrainDump({ imageLocation: response.Location });
  ...
  saveBrainDump(data) {
    ...
    let share = { content: [contentObj] };
    let path = ADD_BRAINDUMP.replace('{}', group._id);
    instance
      .post(path, { ...share })
      .then(res => {
        console.log('BrainDump Save ', res);
      })
      .catch(err => {
