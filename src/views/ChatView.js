...
export default class UserTaskGroupView extends BaseComponent {
    ...
    componentDidMount() {
        console.log('chatview componentDidMount() called');
        super.componentDidMount();
        const {userActions} = this.props;
        //  userActions.getMessageListRequest(this.state.taskGroup._team._id);

        //this.socket.on('client:message', this.onReceivedMessage);
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (Array.isArray(nextProps.messages)) {
            if (nextProps.messages.length > 0) {
                let arrayMessages = getMessages(this.state.taskGroup, nextProps.messages, this.props.user.blockUserIds, this.props.user);
    ...
    setTaskAndTaskGroup() {
        ...
        if (Array.isArray(taskGroup.messages) && taskGroup.messages.length > 0) {
            ...
            this.setState({messages: arrayMessages});
            //  this.saveMessagesInLocalDB(arrayMessages)
    ...
    componentWillUnmount() {
        console.log('chatview componentWillUnmount called');
        let st = this.state;
    ...
    openImagePicker = () => {
        const options = {
            title: 'Select a file',
            //customButtons: [{ name: 'cb', title: 'Custom button' }],
            storageOptions: {
                skipBackup: true,
                path: 'images',
            },
    ...
    uploadSelectedImage = async (challenge) => {
        try {
            ...
            if (!Object.keys(this.state.userTask).length) {
                ...
                if (objectiveType) {
                    //this.createTask(challenge, objectiveType, taskGroupId);
    ...
    goToTask = story => {
        MixPanel.track('User started task', {
        ...
        if (objectiveName === TASK_GROUP_OBJECTIVE.BRAINDUMP || objectiveName.includes(TASK_GROUP_OBJECTIVE.BRAINDUMP)) {
            this.props.navigation.navigate('BrainDumpView',{group: taskGroup});
            return;
        }
        ...
        if (Object.keys(this.state.userTask).length) {
            ...
            console.log('story objective - ', story);
    ...
    onReceivedMessage(message) {
        ...
        if (message.receiver !== groupDetails._team._id) {
            return;
        }
    ...
    render() {
        ...
        return (
            <Fragment>
                ...
                <ImageBackground source = {require('../images/backblue.png')} style = {{width:'100%',height:Platform.OS === 'ios' ? 94 : 57,flex:0}}>
                ...
                {!keyboardShow && <View style={styles.storyDetailView}>
                    ...
                    <View style = {{backgroundColor:'#3c1464',alignContent:'center',justifyContent:'center',padding:10}}>
                                    ...
                                    <View style={
                                        ...
                                        {this.state.processing ? (
                                            <ActivityIndicator size={Platform.OS === 'ios' ? 'small' : 18}
                                                ...
                                                : (
                                                    <Text style={{color: '#ffffff', fontSize: 13,fontFamily:fontFamilyName}}>
